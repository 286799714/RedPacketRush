...
This repository tracks only the Windows x86_64 debug and release binaries,
which are the two targets required by the desktop prototype. The root
`.gitignore` excludes the other platform binaries; re-running the extraction
command restores them when another export target is intentionally added.
