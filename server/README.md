...
要单独运行两个保守型、两个激进型机器人的 1,000 局固定种子统计：

```powershell
npm test -- --grep "Bot strategy Monte Carlo"
```

测试会轮换两类机器人的席位，并在并列时平分一局胜场，然后输出两类机器人的总胜率。
...
等待房间必须正好有四个已准备席位才能开始。房主才能修改设置、填充机器人或开始牌局；牌局开始后房间会变为 private/locked 并从大厅消失。房间按内部席位索引稳定分配机器人策略：偶数席位使用保守型策略，奇数席位使用激进型策略；断线接管沿用同一席位策略。
